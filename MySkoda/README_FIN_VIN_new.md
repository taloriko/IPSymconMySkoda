...
Die FIN-Interpretation ist eine lokale Zusatzfunktion des MySkoda-Moduls. Sie benötigt keine Fahrzeuganfrage und keinen externen Decoder. **Die Angaben sind keine offiziellen Fahrzeugstammdaten von Škoda und kein vollständiger Ausstattungsnachweis.** Die folgenden Tabellen beschreiben die im Modul hinterlegten Zuordnungen, nicht einen für alle Fahrzeuge garantierten Herstellervertrag.
...
Die lokalen FIN-Angaben sind von den [tatsächlich empfangenen API-Daten](README_new.md) getrennt. Auch die JSON-Ausgabe des lokalen Decoders enthält die vollständige FIN und muss vor öffentlicher Weitergabe anonymisiert werden.
